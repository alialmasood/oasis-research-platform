...
import { Wrench, Calendar, Mic, Users } from "lucide-react";
import type { Workshop } from "@prisma/client";

export type WorkshopsStats = {
  total: number;
  last12Months: number;
  asPresenter: number;
...
function calculateStats(workshops: Workshop[]): WorkshopsStats {
  const total = workshops.length;
  
  // ورش العمل خلال آخر 12 شهر
  const twelveMonthsAgo = new Date();
  twelveMonthsAgo.setMonth(twelveMonthsAgo.getMonth() - 12);
  const last12Months = workshops.filter((w) => {
    const workshopDate = new Date(w.date);
    return workshopDate >= twelveMonthsAgo;
  }).length;

  const asPresenter = workshops.filter((w) => w.participationType === "PRESENTER").length;
  const asParticipant = workshops.filter((w) => w.participationType === "PARTICIPANT").length;
  
  const beneficiaries = new Set<string>();
  const beneficiaryCounts: Record<string, number> = {};
  
  workshops.forEach((w) => {
    beneficiaries.add(w.beneficiary);
    beneficiaryCounts[w.beneficiary] = (beneficiaryCounts[w.beneficiary] || 0) + 1;
  });
  ...
  // حسب السنة
  const byYear = workshops.reduce<Record<number, number>>((acc, w) => {
  ...
  const participationByYear = workshops.reduce<Record<number, { presenter: number; participant: number }>>((acc, w) => {
    const y = new Date(w.date).getFullYear();
    if (!acc[y]) acc[y] = { presenter: 0, participant: 0 };
    if (w.participationType === "PRESENTER") acc[y].presenter += 1;
...
export function useWorkshopsStats(workshops: Workshop[]): WorkshopsStats {
  return calculateStats(workshops);
}

export function WorkshopsKPICards({ stats }: { stats: WorkshopsStats }) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
      ...
      <Card className="border border-slate-100 border-r-4 border-r-blue-500 bg-white shadow-lg min-h-[92px]">
        <CardContent className="p-4 h-full flex flex-col justify-between">
          ...
          <p className="text-xs text-slate-500 leading-tight">إجمالي ورش العمل</p>
        </CardContent>
      </Card>
