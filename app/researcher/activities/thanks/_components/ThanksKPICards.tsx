...
import { Heart, Calendar, Mic, Users } from "lucide-react";
import type { ThankYouLetter } from "@prisma/client";

export type ThanksStats = {
  total: number;
  last12Months: number;
  asPresenter: number;
...
function calculateStats(letters: ThankYouLetter[]): ThanksStats {
  const total = letters.length;
  
  // كتب الشكر خلال آخر 12 شهر
  const twelveMonthsAgo = new Date();
  twelveMonthsAgo.setMonth(twelveMonthsAgo.getMonth() - 12);
  const last12Months = letters.filter((l) => {
    const letterDate = new Date(l.date);
    return letterDate >= twelveMonthsAgo;
  }).length;

  const asPresenter = letters.filter((l) => l.participationType === "PRESENTER").length;
  const asParticipant = letters.filter((l) => l.participationType === "PARTICIPANT").length;
  
  const organizations = new Set<string>();
  const organizationCounts: Record<string, number> = {};
  
  letters.forEach((l) => {
    organizations.add(l.issuingOrganization);
    organizationCounts[l.issuingOrganization] = (organizationCounts[l.issuingOrganization] || 0) + 1;
  });
  ...
  // حسب السنة
  const byYear = letters.reduce<Record<number, number>>((acc, l) => {
  ...
  const participationByYear = letters.reduce<Record<number, { presenter: number; participant: number }>>((acc, l) => {
    const y = new Date(l.date).getFullYear();
    if (!acc[y]) acc[y] = { presenter: 0, participant: 0 };
    if (l.participationType === "PRESENTER") acc[y].presenter += 1;
...
export function useThanksStats(letters: ThankYouLetter[]): ThanksStats {
  return calculateStats(letters);
}

export function ThanksKPICards({ stats }: { stats: ThanksStats }) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
      ...
      <Card className="border border-slate-100 border-r-4 border-r-blue-500 bg-white shadow-lg min-h-[92px]">
        <CardContent className="p-4 h-full flex flex-col justify-between">
          ...
          <p className="text-xs text-slate-500 leading-tight">إجمالي كتب الشكر</p>
        </CardContent>
      </Card>
